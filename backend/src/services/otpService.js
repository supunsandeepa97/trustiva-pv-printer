...
async function set(companyId) {
  ...
  await pool.query(
    ...
    [companyId, OTP_KEY, JSON.stringify({ otpHash: hashOtp(otp), expires, attempts: 0 })]
  );
  return otp;
}
...
async function verify(otp, companyId) {
  ...
  if (!otpHashEquals(stored.otpHash, hashOtp(String(otp || '')))) {
    ...
    if (newAttempts >= MAX_ATTEMPTS) {
      ...
      await pool.query(
        ...
        [JSON.stringify({ otpHash: stored.otpHash, expires: stored.expires, attempts: newAttempts }), companyId, OTP_KEY]
      );
    }
    return false;
  }
